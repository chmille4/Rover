...
var JSDAS = function() {
	...
	var jsdas = {
		...
		features: function(url, callback, error_callback, version, args) {
		    var xhr = JSDAS.XMLLoader.getXhrCORS();
			JSDAS.XMLLoader.load(url, function(xml) {
...
JSDAS.XMLLoader = {
	initialize: function() {
		...
		this.proxyURL = undefined; //TODO: this should be settable from the main jsDAS creation function
	},

	//Get a CORS-capable XHR or return null if not available
	xhrCORS: function(method, url, xhr) {
	 ...
	 */
	load: function(url, callback, errorcallback, xhr1){
		if (!this.initialized) {
			this.initialize();
		}
		...
		var xhr
		if (xhr1 != undefined)
		    xhr = this.xhrCORS('GET', url, xhr1);
		else
		    xhr = this.xhrCORS('GET', url);

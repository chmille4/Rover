(function(){var a=!1,b=/xyz/.test(function(){})?/\b_super\b/:/.*/;this.Class=function(){};Class.extend=function(c){function d(){!a&&this.init&&this.init.apply(this,arguments)}var e=this.prototype;a=!0;var f=new this;a=!1;for(var g in c)f[g]=typeof c[g]=="function"&&typeof e[g]=="function"&&b.test(c[g])?function(a,b){return function(){var c=this._super;this._super=e[a];var d=b.apply(this,arguments);this._super=c;return d}}(g,c[g]):c[g];d.prototype=f;d.constructor=d;d.extend=arguments.callee;return d}})();var SCRIBL={};SCRIBL.chars={};SCRIBL.chars.nt_color="white";SCRIBL.chars.nt_A_bg="red";SCRIBL.chars.nt_G_bg="blue";SCRIBL.chars.nt_C_bg="green";SCRIBL.chars.nt_T_bg="black";SCRIBL.chars.nt_N_bg="purple";SCRIBL.chars.nt_dash_bg="rgb(120,120,120)";SCRIBL.chars.heights=[];SCRIBL.chars.canvasHolder=document.createElement("canvas");
...
e.mouseHandler=this.mouseHandler;e.clickHandler=this.clickHandler;e.previousDrawStyle=this.previousDrawStyle;for(var f=0;f<d;f++){var g=this.tracks[f],h=e.addTrack();h.drawStyle=g.drawStyle;for(var i=g.lanes.length,k=0;k<i;k++)for(var l=h.addLane(),j=g.lanes[k].features,m=0;m<j.length;m++){var n=j[m].position+j[m].length,o=j[m].position;if(c=="inclusive")o>=a&&o<=b?l.addFeature(j[m].clone()):n>a&&n<b?l.addFeature(j[m].clone()):o<a&&n>b?l.addFeature(j[m].clone()):o>a&&n<b&&l.addFeature(j[m].clone());
else if(c=="strict")if(o>=a&&o<=b)if(n>a&&n<b)l.addFeature(j[m].clone());else{var p=j[m].glyphType=="BlockArrow"&&j[m].strand=="+"?j[m].clone("Rect"):j[m].clone();p.length=Math.abs(b-o);l.addFeature(p)}else if(n>a&&n<b)p=j[m].glyphType=="BlockArrow"&&j[m].strand=="-"?j[m].clone("Rect"):j[m].clone(),p.position=a,p.length=Math.abs(n-a),l.addFeature(p);else{if(o<a&&n>b)p=j[m].glyphType=="BlockArrow"?j[m].clone("Rect"):j[m].clone(),p.position=a,p.length=Math.abs(b-a),l.addFeature(p)}else c=="exclusive"&&
o>=a&&o<=b&&n>a&&n<b&&l.addFeature(j[m].clone())}}return e},draw:function(){var a=this.ctx,b=this.tracks;this.scrollable==!0&&this.initScrollable();a.save();if(this.scale.pretty){if(this.tick.auto)this.tick.major.size=this.determineMajorTick(),this.tick.minor.size=Math.round(this.tick.major.size/10);if(this.scale.auto)this.scale.min-=this.scale.min%this.tick.major.size,this.scale.max=Math.round(this.scale.max/this.tick.major.size+0.4)*this.tick.major.size}if(this.offset==void 0)this.offset=Math.ceil(a.measureText("0").width/
...
2,e=this.scale.font.size+this.scale.size*0.66,f=this.scale.font.size+this.scale.size*0.33;a.font=this.scale.font.size+"px arial";a.textBaseline="top";a.fillStyle=this.scale.font.color;for(var g=this.scale.min%this.tick.minor.size==0?this.scale.min:this.scale.min-this.scale.min%this.tick.minor.size+this.tick.minor.size;g<=this.scale.max;g+=this.tick.minor.size){a.beginPath();var h=this.pixelsToNts(g-this.scale.min)+this.offset;if(g%this.tick.major.size==0){var i=this.getTickText(g);a.textAlign="center";
a.fillText(i,h,0);a.moveTo(h,c);a.lineTo(h,d);a.strokeStyle=this.tick.major.color}else a.moveTo(h,c),g%(this.tick.major.size/2)==0?(a.strokeStyle=this.tick.halfColor,a.lineTo(h,f)):(a.strokeStyle=this.tick.minor.color,a.lineTo(h,e));a.stroke()}a.fillStyle=b;a.translate(0,this.getScaleHeight()+this.laneBuffer)},pixelsToNts:function(a){return a==void 0?this.width/(this.scale.max-this.scale.min):this.width/(this.scale.max-this.scale.min)*a},ntsToPixels:function(a){return a==void 0?1/this.pixelsToNts():
...
for(var c=this.myMouseEventHandler.mouseY,d,e=0;e<this.tracks.length;e++)for(var f=0;f<this.tracks[e].lanes.length;f++){var g=this.tracks[e].lanes[f].getPixelPositionY(),h=g+this.tracks[e].lanes[f].getHeight();if(c>=g&&c<=h){d=this.tracks[e].lanes[f];break}}if(d){e=d.track.getDrawStyle();if(e=="collapse")this.redraw();else if(e!="line"){this.ctx.save();d.erase();this.ctx.translate(0,d.getPixelPositionY());for(d.draw();e=this.lastToolTips.pop();)this.ctx.putImageData(e.pixels,e.x,e.y);this.ctx.restore()}d=
...
d=this.lanes,e=this.chart.laneBuffer,f=this.chart.trackBuffer,g=c+f,h=this.chart.ctx;if(!a)if(b==void 0||b=="expand")for(b=0;b<d.length;b++)d[b].y=g,d[b].draw(),c=d[b].getHeight(),h.translate(0,c+e),g=g+c+e;else if(b=="collapse"){c=[];for(b=0;b<d.length;b++)c=c.concat(d[b].features);c.sort(function(a,b){return a.position-b.position});for(g=0;g<c.length;g++){a=c[g].length;b=c[g].name;var i=void 0;for(i=g+1;i<c.length;i++)if(c[g].getEnd()>=c[i].position)c[g].length=Math.max(c[g].getEnd(),c[i].getEnd())-
c[g].position,c[g].name="";else break;c[g].draw();c[g].length=a;c[g].name=b;g=i-1}d.length>0&&h.translate(0,d[0].getHeight()+e)}else if(b=="line"){this.coverageData.length==0&&this.calcCoverageData();g=this.maxDepth;h.beginPath();for(a=this.chart.offset;a<=this.chart.width+this.chart.offset;a++)b=this.coverageData[a]/g*c||0,b=c-b,h.lineTo(a,b);h.lineTo(this.chart.width+this.chart.offset,c);h.stroke();h.translate(0,d[0].getHeight()+e)}h.translate(0,f-e)}},addDrawHook:function(a,b){var c=b||_uniqueId("drawHook");
...
svgToString=function(a,b){var c=[];if(a){var d=a.attributes,e,f,g=a.childNodes;for(f=0;f<b;f++)c.push(" ");c.push("<");c.push(a.nodeName);if(a.id=="svgcontent"){f=getResolution();c.push(' width="'+f.w+'" height="'+f.h+'" xmlns="'+svgns+'"');var h={};$(a).find("*").andSelf().each(function(){$.each(this.attributes,function(a,b){var d=b.namespaceURI;d&&!h[d]&&nsMap[d]!=="xmlns"&&nsMap[d]!=="xml"&&(h[d]=!0,c.push(" xmlns:"+nsMap[d]+'="'+d+'"'))})});for(f=d.length;f--;){e=d.item(f);var i=toXml(e.nodeValue);
...
pathActions.convertPath(a,!0)),c.push(e.nodeName),c.push('="'),c.push(i),c.push('"'));if(a.hasChildNodes()){c.push(">");b++;d=!1;for(f=0;f<g.length;f++)switch(e=g.item(f),e.nodeType){case 1:c.push("\n");c.push(svgToString(g.item(f),b));break;case 3:e=e.nodeValue.replace(/^\s+|\s+$/g,"");e!=""&&(d=!0,c.push(toXml(e)+""));break;case 8:c.push("\n"),c.push(Array(b+1).join(" ")),c.push("<\!--"),c.push(e.data),c.push("--\>")}b--;if(!d){c.push("\n");for(f=0;f<b;f++)c.push(" ")}c.push("</");c.push(a.nodeName);
...
drawText:function(a){var b=this.lane.chart.ctx,c=this.getPixelLength(),d=this.getHeight(),e=this.getAttr("text-size"),f=this.getAttr("text-style");b.font=e+"px "+f;b.textBaseline="middle";b.fillStyle=this.getAttr("text-color");var g=void 0,h=this.getAttr("text-align");h=="start"?h=this.strand=="+"?"left":"right":h=="end"&&(h=this.strand=="+"?"right":"left");b.textAlign=h;h=="left"?g=5:h=="center"?g=c/2:h=="right"&&(g=c-5);var i=b.measureText(a);if(a&&a!=""){for(;c-i.width<4;)if(e=/^\d+/.exec(b.font),
e--,i=b.measureText(a),b.font=e+"px "+f,e<=8){a="";break}this.glyphType=="Complex"&&(c=0,e=/^\d+/.exec(b.font),h=="center"&&(c=-(b.measureText(a).width/2+2.5)),b.clearRect(g+c,d/2-e/2,b.measureText(a).width+5,e));b.fillText(a,g,d/2)}},calcRoundness:function(){return this.getHeight()*this.getAttr("roundness")/100},isContainedWithinRect:function(a,b,c,d){var e=this.getPixelPositionY(),f=this.getPixelPositionX(),g=this.getPixelPositionX()+this.getPixelLength(),h=e+this.getHeight();return f>=a&&g<=c&&
e>=b&&h<=d},getHeight:function(){return this.lane.getHeight()},getFillStyle:function(){var a=this.getAttr("color");if(a instanceof Array){for(var b=this.lane.ctx.createLinearGradient(this.length/2,0,this.length/2,this.getHeight()),c,d=0;c=a[d],d<a.length;d++)b.addColorStop(d/(a.length-1),c);return b}else return a instanceof Function?(b=this.lane.ctx.createLinearGradient(this.length/2,0,this.length/2,this.getHeight()),a(b)):a},getStrokeStyle:function(){var a=this.getAttr("borderColor");if(typeof a==
...
g=SCRIBL.chars;if(!g.heights[c]){g.heights[c]=[];for(var h=0;h<this.chars.list.length;h++){var i=this.chars.list[h],k=i;i=="-"&&(k="dash");this.createChar(i,g.nt_color,g["nt_"+k+"_bg"],c)}}x=y=0;if(this.imgCanvas)a.drawImage(this.imgCanvas,e,f-c*d,b,c*d);else{a.save();a.beginPath();a.textBaseline="middle";e=a.font;h=/[\d+px]/.exec(e)+"px";a.font=h+" courier";a.fillStyle="black";a.textAlign="left";h=this.seq.length*g.heights[c].width;this.imgCanvas=document.createElement("canvas");this.imgCanvas.width=
h;this.imgCanvas.height=c;f=this.imgCanvas.getContext("2d");for(h=k=i=0;h<this.seq.length;h++){g.heights[c][this.seq[h]]||this.createChar(this.seq[h],"black","white",c);var l=this.seq[h];this.insertions[k]&&this.insertions[k].pos!=void 0&&(this.insertions[k].pos-1==h?l+="rightInsert":this.insertions[k]&&this.insertions[k].pos==h&&(l+="leftInsert",k++));f.drawImage(g.heights[c][l],i,y);i+=g.heights[c].width}a.drawImage(this.imgCanvas,x,c-c*d,b,c*d);a.font=e;a.restore()}a.beginPath();a.moveTo(0,0);
a.lineTo(b,y);a.lineTo(b,y+c);a.lineTo(x,y+c);a.lineTo(x,y);a.fillStyle="rgba(0,0,0,0)";a.strokeStyle=this.lane.chart.ntsToPixels()<=this.fractionLevel?"rgba(0,0,0,1)":"rgba(0,0,0,0)";a.stroke();a.closePath()},createChar:function(a,b,c,d){var e=document.createElement("canvas"),f=e.getContext("2d"),g=d-2;f.font=g+"px courier";var h=f.measureText(a).width+2;e.height=d;e.width=h;SCRIBL.chars.heights[d].width=h;e=document.createElement("canvas");f=e.getContext("2d");f.font=g+"px courier";e.height=d;e.width=
h;var i=f.fillStyle;f.fillStyle=c;f.fillRect(0,0,h,d);f.fillStyle=b;f.textAlign="center";f.textBaseline="middle";f.fillText(a,h/2,d/2);SCRIBL.chars.heights[d][a]=e;f.fillStyle=i;e=document.createElement("canvas");f=e.getContext("2d");f.font=g+"px courier";e.height=d;e.width=h;i=f.fillStyle;f.fillStyle=c;f.fillRect(0,0,h,d);f.fillStyle="yellow";f.beginPath();f.moveTo(0,d);f.arcTo(h,d,h,0,d/2);f.lineTo(h,d);f.lineTo(0,d);f.closePath();f.fill();f.fillStyle=b;f.textAlign="center";f.textBaseline="middle";
f.fillText(a,h/2,d/2);SCRIBL.chars.heights[d][a+"rightInsert"]=e;f.fillStyle=i;e=document.createElement("canvas");f=e.getContext("2d");f.font=g+"px courier";e.height=d;e.width=h;i=f.fillStyle;f.fillStyle=c;f.fillRect(0,0,h,d);f.fillStyle="yellow";f.beginPath();f.moveTo(h,d);f.arcTo(0,d,0,0,d/2);f.lineTo(0,d);f.lineTo(h,d);f.closePath();f.fill();f.fillStyle=b;f.textAlign="center";f.textBaseline="middle";f.fillText(a,h/2,d/2);SCRIBL.chars.heights[d][a+"leftInsert"]=e;f.fillStyle=i}});var Line=Glyph.extend({init:function(a,b,c,d){this.thickness=2;this._super(a,b,c,void 0,d);this.glyphType="Line"},_draw:function(a,b,c){a=a||this.ctx;b=b||this.getPixelLength();c=c||this.getHeight();x=y=0;a.beginPath();a.moveTo(x,c/2-this.thickness/2);a.lineTo(x,c/2+this.thickness/2);a.lineTo(x+b,c/2+this.thickness/2);a.lineTo(x+b,c/2-this.thickness/2)}});var Complex=Glyph.extend({init:function(a,b,c,d,e,f){this._super(a,b,c,d,f);this.slope=1;this.glyphType="Complex";this.subFeatures=e;this.line=new Line(a,0,c);this.line.parent=this;this.line.color="black";this.line.thickness=2},addSubFeature:function(a){this.subFeatures.push(a)},_draw:function(a,b,c,d){a=a||this.ctx;b||this.getPixelLength();c||this.getHeight();d+1||this.calcRoundness();x=y=0;a.translate(-this.getPixelPositionX(),0);this.line.lane=this.lane;this.line.draw();b=this.subFeatures.length;
